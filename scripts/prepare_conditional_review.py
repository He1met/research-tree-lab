#!/usr/bin/env python3
"""Offline conditional review preparation; no commit, network, publish or native action."""
import argparse
from pathlib import Path
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from researchlib.common import canonical, read_json
from researchlib.conditional_review import prepare_outbox
...
def main():
    ...
    try:
        ...
        print(canonical({'state': 'REJECTED', 'reason_code': 'CONDITIONAL_PREPARATION_FAILED'}).decode(), file=sys.stderr, end='')
        return 2
